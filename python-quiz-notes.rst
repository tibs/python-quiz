...
.. note:: I thought of Perl, Perl6, PHP, Pascal, Prolog, ...
...
.. note:: I knew occam, and F#, and Haskell, and should have thought of Boo.

.. note:: With some caveats
...
.. note:: In some programming languages, the "power" operator is ``^``.
...
.. note:: The order matters - the ``else`` must come before the ``finally``.
...
.. note:: as the logging callable will only construct the final string if the
  log message is actually output. In the other two examples, the final string
  is created when the ``logger.info`` call is made, even if the callable
  decides not to output anything.
...
.. note::  Basically, the ``except`` clause overwrote the value of ``e``,
  and then cleared it.

  We want the value to "go away" because an exception has a traceback
  attached to it, which refers to the stack frame for the current function,
  which in turn refers to the local variables for that function, keeping them
  alive.

  If the exception had not been caught, then we wouldn't have set ``e``, and
  nor would it have been cleared.
...
.. note:: In Python, Decimal characters include digit characters, and all
  characters that can be used to form decimal-radix numbers, e.g. ``U+0660,
  ARABIC-INDIC DIGIT ZERO``. See `adventures in unicode digits`__ for some more
  information.
...
#. We need it as a method argument because it doesn't have to be called
   ...
   ``<class_name>.<method_name>(<instance>, ...)``) then it helps to have an
   explicit place in the argument list for it. Although this is an edge case,
   and one could argue that it doesn't of itself *require* having `self`
